...
"07-reranking": [
 ...
 "Because embeddings encode **meaning, not truth**. That sentence uses the same vocabulary and the same grammatical shape as the correct answer (\"the capital of X is Y\"), so a model trained to place similar-meaning texts together puts it right next to the query. Nothing in a vector records whether a statement is factually right, so cosine similarity cannot filter it out — only a model that judges answer quality (a re-ranker) or a downstream fact-check can.",
 "Random negatives are too easy — telling \"capital of Canada\" apart from an unrelated passage teaches the model almost nothing. **Hard negatives** are the near-misses the re-ranker actually faces in production (topically close, plausibly worded, but wrong), so training on them is what teaches the distinction between \"similar\" and \"answers the query\". They are typically mined by running a first-stage retriever and taking its top-ranked wrong results.",
],
"08-hybrid-search": [
